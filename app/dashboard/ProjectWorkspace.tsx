...
export default function ProjectWorkspace({ projectId }: { projectId: string }) {
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener("popstate",syncSection); window.removeEventListener("storage", storage); window.removeEventListener("keydown", shortcut); };
  }, [projectId]);
  ...
  return <div className={`studio project-workspace ${section === "Characters" ? "characters-workspace" : ""}`}>
    ...
    <main className="studio-main"><div className="pw-banner" style={{backgroundImage:`linear-gradient(90deg,#000d18f5 0%,#000d18a8 38%,#000d1820 75%),linear-gradient(0deg,#000d18,transparent 70%),url("${art(project?.cover)}")`}}/>
      <header className="studio-topbar"><div className="studio-breadcrumb"><button className="studio-icon mobile-only" aria-label="Open navigation" onClick={()=>setMenu(true)}><Icon name="Menu"/></button><Icon name="Assets" size={19}/><Link href={section === "Characters" ? `/dashboard/projects/${encodeURIComponent(projectId)}` : "/dashboard/projects"}>{section === "Characters" ? project?.title || "Your project" : "Projects"}</Link><span className="crumb-separator">›</span><span className="pw-crumb-title">{section === "Characters" ? "Characters" : project?.title || "Your project"}</span></div><div className="studio-tools"><label className="studio-search"><Icon name="Search" size={17}/><input ref={search} value={query} onChange={e=>setQuery(e.target.value)} placeholder={section === "Characters" ? "Search characters, traits, or lore…" : "Search this world…"} aria-label="Search this project"/><kbd>⌘ K</kbd></label><button className="studio-icon" aria-label="Recent activity" onClick={()=>{navigate("Activity");setAllActivity(true);}}><Icon name="Bell"/></button><Link className="studio-avatar" href="/dashboard" aria-label="Return to studio"><img src={art("action")} alt=""/></Link></div></header>
      {!loaded ? <div className="pw-missing" role="status">Opening your world…</div> : !project ? <div className="pw-missing"><h1>Project not found in this browser.</h1><p>Open this project on the browser where you created it, or start a new world.</p><Link className="project-primary" href="/dashboard/projects">Back to My Projects</Link></div> : <>
        {section === "Characters" ? <CharactersView characters={entries.filter(e=>e.kind === "Characters")} query={query} onQuery={setQuery} onCreate={()=>editEntry("Characters")} onEdit={entry=>editEntry("Characters",entry)} notice={notice}/> : <><section className="pw-hero"><div><p className="pw-meta">{project.format}<span>•</span>{project.style}<span>•</span>{project.status || "Draft"}</p><h1>{project.title}</h1><p className="pw-description">{project.idea}</p></div><button className="project-secondary" onClick={()=>edit("project")}><Icon name="Edit" size={17}/> Edit Project</button></section>
