...
export default function ProjectWorkspace({ projectId }: { projectId: string }) {
  ...
  function save() {
    ...
    if (editor === "entry") {
      const entry: WorldEntry = {...(entryId ? entries.find(e=>e.id===entryId) : {}),id:entryId || crypto.randomUUID(),kind:entryKind,title:title.trim(),description:body.trim(),cover,status:entryStatus,updatedAt:new Date().toISOString(),...(entryKind === "Characters" ? {characterRole,characterState,characterTraits:characterTraits.trim(),characterProfile} : {})};
  ...
  return <div className={`studio project-workspace ${section === "Characters" ? "characters-workspace" : isBible ? "bible-workspace" : ""}`}>
    ...
    <main className="studio-main"><div className="pw-banner" style={{backgroundImage:`linear-gradient(90deg,#000d18f5 0%,#000d18a8 38%,#000d1820 75%),linear-gradient(0deg,#000d18,transparent 70%),url("${art(project?.cover)}")`}}/>
      <header className="studio-topbar"><div className="studio-breadcrumb"><button className="studio-icon mobile-only" aria-label="Open navigation" onClick={()=>setMenu(true)}><Icon name="Menu"/></button><Icon name="Assets" size={19}/><Link href={(section === "Characters" || isBible) ? `/dashboard/projects/${encodeURIComponent(projectId)}` : "/dashboard/projects"}>{(section === "Characters" || isBible) ? project?.title || "Your project" : "Projects"}</Link><span className="crumb-separator">›</span><span className="pw-crumb-title">{(section === "Characters" || isBible) ? (section === "Story Bible" ? "Story Bible" : `Story Bible › ${section}`) : project?.title || "Your project"}</span></div><div className="studio-tools"><label className="studio-search"><Icon name="Search" size={17}/><input ref={search} value={query} onChange={e=>setQuery(e.target.value)} placeholder={section === "Characters" ? "Search characters, traits, or lore…" : "Search this world…"} aria-label="Search this project"/><kbd>⌘ K</kbd></label><button className="studio-icon" aria-label="Recent activity" onClick={()=>{navigate("Activity");setAllActivity(true);}}><Icon name="Bell"/></button><Link className="studio-avatar" href="/dashboard" aria-label="Return to studio"><img src={art("action")} alt=""/></Link></div></header>
      {!loaded ? <div className="pw-missing" role="status">Opening your world…</div> : !project ? <div className="pw-missing"><h1>Project not found in this browser.</h1><p>Open this project on the browser where you created it, or start a new world.</p><Link className="project-primary" href="/dashboard/projects">Back to My Projects</Link></div> : <>
        {isBible ? <StoryBibleView key={`${project.id}:${section}`} initialCategory={bibleSections[section]} onNavigate={navigate} project={project} entries={entries} query={query} onQuery={setQuery} onSave={persist} onSummary={()=>edit("overview")} onEdit={entry=>editEntry(entry.kind,entry)} notice={notice}/> : section === "Characters" ? <CharactersView characters={entries.filter(e=>e.kind === "Characters")} query={query} onQuery={setQuery} onCreate={()=>editEntry("Characters")} onEdit={entry=>editEntry("Characters",entry)} notice={notice}/> : <><section className="pw-hero"><div><p className="pw-meta">{project.format}<span>•</span>{project.style}<span>•</span>{project.status || "Draft"}</p><h1>{project.title}</h1><p className="pw-description">{project.idea}</p></div><button className="project-secondary" onClick={()=>edit("project")}><Icon name="Edit" size={17}/> Edit Project</button></section>
